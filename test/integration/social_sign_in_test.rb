...
class SocialSignInTest < ActionDispatch::IntegrationTest
  ...
  def with_auth_hash(uid:, email:, name:, verified: true)
  ...
  test "first Google sign-in creates a user, workspace org and identity" do
    with_auth_hash(uid: "g-123", email: "Nina@Example.com", name: "Nina Park") do
  ...
  test "returning identity signs in without creating anything" do
    ...
    user.identities.create!(provider: "google_oauth2", uid: "g-ret")

    with_auth_hash(uid: "g-ret", email: user.email_address, name: "Ret") do
  ...
  test "matching verified email links Google to the existing account" do
    ...
    with_auth_hash(uid: "g-link", email: user.email_address.upcase, name: nil) do
    ...
    assert_equal user.id, Identity.find_by(provider: "google_oauth2", uid: "g-link").user_id
  end

  test "an unverified email is treated as missing" do
    with_auth_hash(uid: "g-unverified", email: "spoof@test.com", name: "Spoof", verified: false) do
  ...
  test "the Google button renders only when credentials are configured" do
    get new_session_path
    assert_response :success
    refute_match "auth/google_oauth2", response.body, "no button without credentials"
