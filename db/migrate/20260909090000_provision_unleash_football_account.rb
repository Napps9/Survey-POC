# Data migration: create the Unleash Football client account and Jamie's and
...
# the ordinary migration path, exactly as ProvisionAlpbachAccount does.
#
# Idempotent and credential-free; all the reasoning lives on
# UnleashFootballAccountProvisioner. Up-only: withdrawing an account is a
...
class ProvisionUnleashFootballAccount < ActiveRecord::Migration[8.1]
  def up
    ...
    say "Unleash Football account provisioning skipped: #{e.class}: #{e.message}"
  end
