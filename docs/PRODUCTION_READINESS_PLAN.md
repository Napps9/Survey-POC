...
| P1-6 | **Back up AR encryption keys** — **still an ops task, but the fallout is now survivable.** The three `ACTIVE_RECORD_ENCRYPTION_*` keys are `generateValue: true`: Render mints them once for a service, never backs them up, and generates new ones if the service is recreated — at which point every stored Google token is encrypted with a key that no longer exists. **Verified, not assumed:** written under one key and read under another, `User#google_connected?` raises `ActiveRecord::Encryption::Errors::Decryption` — and that method is called from `surveys/index.html.erb`, the **dashboard**, so losing the keys 500'd the app's home page for every affected user, including the page they'd have used to reconnect. **Done in-repo:** an undecryptable token now degrades to "not connected" and reports to Sentry, so the loss costs everyone a Google reconnect instead of the app; `ACTIVE_RECORD_ENCRYPTION_PREVIOUS_KEYS` makes rotation possible without a re-encryption window (wired in an initializer, not `application.rb` — `DerivedSecretKeyProvider` derives in its constructor from a config that isn't applied yet there, which raises at boot); `rake encryption:fingerprints` verifies a backup against production without either being printed, and `rake encryption:verify` counts undecryptable tokens. The `render.yaml` comment claiming "no manual key handling needed" is corrected — it was the reassurance that stopped anyone checking. **Outstanding, and only the owner can do it:** copy the three values into a secret store. `docs/ENCRYPTION_KEYS.md` is the runbook. | S | `docs/ENCRYPTION_KEYS.md`, `user.rb`, `render.yaml` |
...
| ~~P2-2~~ | ~~**Close i18n leaks**~~ **Done** — 116 hardcoded English flash messages across 27 controllers moved into a `flash:` namespace and translated into all 19 locales, so a non-English creator no longer gets an English confirmation or refusal at the end of an otherwise localized task. A parity test pins the properties nobody can eyeball across 19 languages: every locale has every key, none has a key English doesn't, placeholders match **exactly** (a dropped `%{name}` raises `I18n::MissingInterpolationArgument` in production, in front of whoever triggered the flash), pluralized entries carry the CLDR categories their own language needs (Arabic all six, Russian/Ukrainian/Polish four, Japanese/Korean/Vietnamese/Indonesian one), and the product name survives translation. It also asserts a **floor** on the English key count, because every other assertion in it is a set comparison against English — a truncated `en.yml` would otherwise make the whole file pass while proving nothing. **Three bugs came out of this, all logged:** a class-body `CONSTANT = t(...)` that dev never evaluates and production does (BUG-011, would have failed the deploy at boot), a key added after the translation fan-out had already been dispatched (BUG-012), and a commit whose message misdescribes its contents because the working tree was used as scratch space while agents held it (BUG-010). | M | `flash:` in all 19 `config/locales/*.yml`, 27 controllers, `locale_flash_parity_test.rb` |
...
| ~~P2-4~~ | ~~**Accessibility audit of the player**~~ **Answer controls and deck navigation fixed** — choice options were `<li>`s with a click handler, the range slider was drag-only, rating stars were unlabelled `<span>`s, and **nothing moved focus when the card changed**, so advancing dropped focus to `<body>` and meant Tabbing from the top of the document on every question. All now carry the right role, `tabindex`, live ARIA state and keyboard activation, and focus follows the deck (skipped on first render; not moved when a required card refuses to advance). Applied outside the editor only. Verified in Chromium end-to-end: answer a question by keyboard, advance, focus lands on the new card. **Still open:** the regions map, and there is no browser-test harness to pin any of the JS behaviour — see P2-5. | M | `_card_component.html.erb`, `picker_controller.js`, `slider_controller.js`, `rating_controller.js`, `player_controller.js` |
| ~~P2-5~~ | ~~**Add browser/system tests**~~ **Harness added, wired into CI, first suite covers the player's keyboard path** — Capybara + Cuprite (CDP, so no chromedriver to version-match, and one config works both on the GitHub runner and in a Playwright-only container). `test/system` stays out of `bin/rails test` and runs as its own CI job, so browser flakiness can't take the unit suite — and therefore the deploy — down. Seven tests pin the P2-4 behaviour that could previously only be verified by hand. **Each was checked against a deliberately broken build**: the first version passed with the key handlers disabled, because Cuprite's `send_keys` clicks the element to focus it and the click handler fired instead; keys are now delivered to a DOM-focused element with no pointer involved. **The editor is now covered too**, and it paid for itself on its first run: four tests on the rename/autosave path found BUG-013, a blank rename silently erasing a Verto's name past a guard written specifically to prevent it. That defect lives in the ordering of two DOM event handlers, so no unit test could have seen it — which is the argument for this harness, made concrete. **Still open:** the results screens and the remaining creator flows. | M | `test/application_system_test_case.rb`, `test/system/player_keyboard_test.rb`, `test/system/editor_autosave_test.rb`, `ci.yml` |
| P2-6 | **Onboarding polish** — **the fail-loudly half is done** (see below); the welcome email and the one-click sample Verto are still open. `DemoSeeder` cannot be reused as-is for the latter: it pins fixed org/user slugs as constants, starts by `destroy_existing!`-ing them, and calls `abort` when DEMO_PASSWORD is missing — `SystemExit` inside a Puma thread. A per-org sample means refactoring it to take an org and return values instead of printing, plus a route, a button on the empty state, and 19 locale files. **Done (P2-6a):** a getting-started `WelcomeMailer`, sent when the address is **confirmed** rather than at signup — signup already sends the P0-8 confirmation, and a second mail alongside it would double the volume landing on an address nobody has yet proved is deliverable, which is the worst first impression to give a spam filter. Waiting also means it arrives at a proven address in the same minute someone clicked a link in their inbox. It is the first localized mail in the app: a mailer runs in a Solid Queue job with no request and therefore no `Current.locale`, so the recipient's `preferred_locale` is applied explicitly — without that it renders in whatever locale the worker was last left in. The older mailers remain hardcoded English; converting them is the follow-up. **Done (P2-6c):** `lib/mail_config_check.rb` — a production boot missing `SMTP_ADDRESS`, or missing both `APP_HOST` and `RENDER_EXTERNAL_HOSTNAME`, now logs *and* reports to Sentry instead of writing one `logger.warn` that scrolls off the dashboard in seconds. That silence mattered more than it sounds: every caller rescues a delivery failure and still reports success, so one missing variable loses every password reset, invite, account-setup link and email confirmation with no symptom anywhere. `STRICT_MAIL_CONFIG=1` upgrades it to a boot failure — deliberately opt-in, because turning it on before the production values are known-good converts a fixable misconfiguration into an outage. The check runs in `after_initialize`, not the initializer body: initializers run alphabetically and `mailer` sorts before `sentry`, so a report from there would only ever have reached the log. | M | `lib/mail_config_check.rb`, `mailer.rb`, `demo_seeder.rb` |
| P2-7 | ~~**Migrate `json` → `jsonb`**~~ **Recommend NOT doing this — audited 2026-07-31.** The stated benefit does not exist here and the migration actively breaks working code. **Breaks:** `blazer_starter_queries.rb:188,208,270` call `json_array_elements`, which is json-only — Postgres will not implicitly cast jsonb for function resolution, so all three error at runtime, and their tests only assert the SQL *string* contains `json_array_elements`, so the suite would go green on a change it cannot validate. It also changes object key order, which changes the deck fingerprint at `verto_generation.rb:42` — an in-flight `FinishVertoSetupJob` would mismatch across the deploy and drop that Verto's translations (fails safe, but real). **No benefit:** there are zero indexes on any of the 15 json columns and zero queries filter on JSON content — every filter dimension was deliberately denormalised out into real indexed columns instead (`AddDemographicsToResponses` says so explicitly), so a GIN index would be used by nothing. The three queries that do read JSON are already keyed by primary key. **And it is unverifiable here:** dev, test and CI are all SQLite, so nothing dialect-specific has any coverage at all — which is the more useful finding, and the one worth acting on if any of this is revisited. | M | `schema.rb`, `blazer_starter_queries.rb` |
...
| P2-9 | **Staging environment + documented rollback** (ops, outside this repo); tighten CSP (`script-src` off `'unsafe-inline'` via nonces). **Done: `robots.txt` + `noindex`.** Every unauthenticated path the app exposes is a *capability* — `/play/:token` is how a respondent answers a Verto and the token is the whole of the authorisation, so an indexed URL publishes the capability rather than just a page. Judged from the product's own copy: every distribution mechanism it ships (copy-link, QR for posters, per-partner links, respondent pass-it-on) is possession-of-URL, and nothing anywhere claims search discoverability. `public/robots.txt` now disallows `/play/`, `/invites/`, `/funder_invites/` and `/blazer`, and — because robots.txt is advisory and only reaches a crawler that fetches it — `ApplicationController#discourage_indexing` sets `X-Robots-Tag: noindex, nofollow` on the same paths, which is the half that travels with the response when a link leaks into a tweet or a Slack unfurl. Matched on path, not controller, so the mounted Blazer engine is covered too. Pinned by `search_indexing_test.rb`, including the negative case: ordinary pages must NOT carry the header, or the marketing pages would quietly de-index themselves. | M | `render.yaml`, `content_security_policy.rb:33`, `public/robots.txt`, `application_controller.rb` |
...
- **i18n depth.** 19 locales, 500+ keys each, parity-enforced by tests; player is
  RTL-aware. Creator-side flash messages are localized too, as of P2-2.
