...
| ~~P2-4~~ | ~~**Accessibility audit of the player**~~ **Answer controls fixed; audit not exhaustive** — the choice options were `<li>` elements with a click handler: not focusable, and announced as plain list items with nothing conveying that they were choices or which was chosen. A keyboard-only or screen-reader respondent **could not answer** `multiple_choice`, `select_many`, `yes_no` or either grid — most of what anyone is ever asked. They now carry `role=radio`/`checkbox`, `tabindex=0`, `aria-checked` kept in step with selection, a `radiogroup`/`group` wrapper, and Enter/Space activation; the decorative icon tile is `aria-hidden`. Applied outside the editor only, where the same elements hold contenteditable text. Verified in Chromium with keyboard-only input. **Still open:** this covered the answer controls, not the whole player — the NPS slider, the range/rating cards, the regions map and focus management on card transitions have not been audited. | M | `_card_component.html.erb`, `picker_controller.js` |
