...
| 22 | 80 (4 gen × 20) — **branded, two boxes** | **Images flawless, journey unchanged** — combined ~1,120 req/s; 141k image loads, 100% `image 200`, image med 251 ms / p95 2.6 s; journey: show 1.4 s, progress 1.9 s, leaderboard 2.0 s, submit **2.7 s med / 5.3 s p95 / 29 s p99**; 0.00% failed (2 stray 5xx in 400k); ~2,500 dropped, VUs at the 800 cap | First run on the bucket (`docs/OBJECT_STORAGE_CUTOVER.md` phases 0–2 rehearsed and smoke-tested on scratch) with TWO 12c-24g instances (manual scaling) and the branded deck (logo + six 60 KB card images, `IMAGES=6`). The object-storage path is proven at scale. But the journey endpoints are no faster than ONE box in run 21, and the images — a Rails 302 each before the browser reaches R2 — were 60% of all requests. Two readings: the second instance not yet in rotation, or the wall has moved to a tier both boxes share. |
| 23 | 80 (4 gen × 20) — **two boxes, `SKIP=images`** | **Identical to one box** — combined ~435 req/s, med 1.9 s, p95 5.1 s (show 2.1 / 5.1, submit 2.2 / 5.5), 0 failed, ~270 dropped per generator | Run 22 minus the attachments: the text-only load of run 21 on twice the web tier, and the same numbers (446 req/s, 1.96 s med). **Doubling the web tier changed nothing, so the wall is not web CPU.** It is a shared resource — the Key Value store (the rate limiter and the ~150 KB page-cache read sit on every request, and the store fails OPEN, so a struggling instance re-renders every page and hands the web tier its biggest cost back) or the database — or a second instance that never took traffic; Render's per-instance, database and Key Value graphs for 12:53–12:58 UTC decide. Shipped regardless, because both are right at 50k: card images and the logo as **presigned bucket URLs baked into the cached page** (`PlayerAssetUrls`, c7f7047 — zero Rails requests per image) and a **process-local front cache for the play page** (one render per Puma worker per hour; the shared store off the page's hot path). |
