module ResolvesResultSegments
  ...
  # Slices by the set demographic questions plus the opt-in Heritage one, from
  # the denormalised columns (see AddDemographicsToResponses for why they aren't
  # read out of the answers JSON). Suppressed below the small-cell threshold,
  # like regions.
  def demographic_segments(base)
    gender_segments(base) + age_segments(base) + heritage_segments(base)
  end
