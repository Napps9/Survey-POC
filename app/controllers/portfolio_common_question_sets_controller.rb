class PortfolioCommonQuestionSetsController < ApplicationController
  ...
  def create
    ...
    redirect_to funder_portfolio_path(@funder, @portfolio), notice: t("flash.portfolio_common_question_sets.attached", name: set.name, portfolio: @portfolio.name)
  end

  def destroy
    pcqs = @portfolio.portfolio_common_question_sets.find(params[:id])
    pcqs.destroy!
    redirect_to funder_portfolio_path(@funder, @portfolio), notice: t("flash.portfolio_common_question_sets.removed", portfolio: @portfolio.name)
  end
  ...
  def load_funder
    ...
    redirect_to funders_path, alert: t("flash.portfolio_common_question_sets.funder_not_found") and return unless @funder
  end

  def load_portfolio
    @portfolio = @funder.portfolios.kept.find_by(id: params[:portfolio_id])
    redirect_to funder_path(@funder), alert: t("flash.portfolio_common_question_sets.portfolio_not_found") and return unless @portfolio
  end

  def require_funder_owner!
    return if @funder.organisation_id == current_organisation.id && current_membership&.admin?
    redirect_to funder_path(@funder), alert: t("flash.portfolio_common_question_sets.funder_only")
  end
end
