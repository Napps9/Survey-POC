class PortfolioMembershipsController < ApplicationController
  ...
  def create
    ...
    redirect_to funder_portfolio_path(@funder, @portfolio),
                notice: t("flash.portfolio_memberships.organisation_added", org: funder_membership.organisation.name, portfolio: @portfolio.name)
  ...
  def destroy
    ...
    membership.destroy!
    redirect_to funder_portfolio_path(@funder, @portfolio), notice: t("flash.portfolio_memberships.organisation_removed", portfolio: @portfolio.name)
  end
  ...
  def load_funder
    ...
    redirect_to funders_path, alert: t("flash.portfolio_memberships.funder_not_found") and return unless @funder
  end

  def load_portfolio
    @portfolio = @funder.portfolios.kept.find_by(id: params[:portfolio_id])
    redirect_to funder_path(@funder), alert: t("flash.portfolio_memberships.portfolio_not_found") and return unless @portfolio
  end

  def require_funder_owner!
    return if @funder.organisation_id == current_organisation.id && current_membership&.admin?
    redirect_to funder_path(@funder), alert: t("flash.portfolio_memberships.funder_only")
  end
end
